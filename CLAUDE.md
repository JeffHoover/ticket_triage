...
| 7 | Observability / governance | Done | Every tool call, escalation, and validation failure is logged as a structured JSON line (`event`, ISO-8601 UTC `timestamp`, domain-specific fields). Pydantic serialization enforces schema; unknown types raise `TypeError` (fail-loud). Log path configurable via `TICKET_TRIAGE_LOG_PATH`. |
...
116 tests across gates, dispatch, escalate, retry, observability, write_audit_event, find_order_by_id, issue_refund, MCP server, billing/technical/refund agents, refund threshold hook, history cap, docs chunking, RAG retrieval, search_docs wiring, e2e dispatch, model routing, `_dispatch_tool_call` unit tests, AgentOutcome discriminated-union variant tests, and Decimal money type tests. 98% line coverage, 74% mutation kill rate (see README for how to run).
