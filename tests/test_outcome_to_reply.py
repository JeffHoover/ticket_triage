...
from ticket_triage.schemas import NeedsInfoOutcome, ResolvedOutcome


def test_resolved_outcome_becomes_resolved_reply():
    outcome = ResolvedOutcome(reply_draft="Your issue is fixed.")
...
def test_needs_info_outcome_becomes_needs_info_reply():
    outcome = NeedsInfoOutcome(reply_draft="Can you provide more details?")
    reply = outcome_to_reply(outcome)
